...
function PlanetaryProductionPage() {
    ...
    useEffect(() => {
        query.longestPath(baseSystem).then(m => setDistanceMax(m))
    }, [baseSystem]);
    useEffect(() => {
        query.getResources().then(r => setResourceNames(r))
    }, []);
    useEffect(() => {
        query.getSystems().then(s => setSystems(s))
    }, []);
    ...
    const distanceMarks = useMemo(() => {
        const marks = [...Array(Math.round(distanceMax / 10)).keys()].map((value) => ({
            value: value * 10,
            label: value * 10
        }));
    ...
    if (systems.length === 0) {
        return ( <
            ...
            gutterBottom > Loading... < /Typography> <
            div className = {
                classes.test
            } >
            <
    ...
    return ( <
        ...
        width = {
            1
        } > EVE Echoes < /Typography> <
        Typography variant = "h5"
        width = {
            1
        } > Planetary Production < /Typography> <
        /Grid>

        <
        Autocomplete disableClearable getOptionLabel = {
            (option) => systems[option].label
        }
        label = "Base System"
        onChange = {
            (event, value) => setBaseSystem(value)
        }
        options = {
            systems.map((system, index) => index)
        }
        value = {
            baseSystem
        }
        ...
        <
        Slider label = "Distance to System (jumps)"
        marks = {
            distanceMarks
        }
        max = {
        ...
        onChange = {
            (event, value) => setDistanceRange(value)
        }
        step = {
            1
        }
        ...
        onChange = {
            (event, value) => setSecurityRange(value)
        }
        step = {
            0.1
        }
        ...
        <
        SelectWithChips id = "resources"
        label = "Resource Types"
        onChange = {
            (event, value) => setResources(value)
        }
        options = {
            resourceNames
        }
        ...
        <
        Slider label = "Resource Richness"
        marks = {
            [{
                value: 25,
                label: '25'
        ...
        onChange = {
            (event, value) => setRichnessRange(value)
        }
        step = {
            1
        }
        ...
        <
        Slider label = "Planets (Planetology max development)"
        marks = {
            [{
                value: 1,
                label: '1'
        ...
        onChange = {
            (event, value) => setPlanetology(value)
        }
        step = {
            1
        }
        ...
        <
        Slider label = "Max Round Trip length (jumps)"
        marks = {
        ...
        onChange = {
            (event, value) => setRoundTripJumps(value)
        }
        step = {
            1
        }
